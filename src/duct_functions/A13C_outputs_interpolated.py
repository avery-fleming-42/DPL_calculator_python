...
from interpolation_manager import get_case_interpolator
...
def A13C_outputs(stored_values, *_):
    ...
    try:
        ...
        # ---------- Base C via interpolator ----------
        bundle = get_case_interpolator("A13C")
        if bundle is None:
            # Fallback to original table-based logic if not preloaded
            ...
            # IDW 2D interpolation: continuous in angle and As/A
            C_base = float(interp(angle, A_ratio))
