...
def A11N_outputs(stored_values, *_):
    """
    Calculate outputs for A11N case.
    ...
    Inputs:
        stored_values: dict with keys:
            ...
            entry_5: Main Upstream / Converged Flow Rate (Qc, cfm)
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11N)
        # ==========================
        try:
            branch_data = get_case_table("A11N")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11N branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11N."}

        branch_data["Vb/Vc Diff"] = (branch_data["Vb/Vc"] - Vb_Vc).abs()
        branch_data["Qb/Qc Diff"] = (branch_data["Qb/Qc"] - Qb_Qc).abs()
...
A11N_outputs.output_type = "branch_main"
