...
def A11J_outputs(stored_values, *_):
    """
    Calculate outputs for A11J case.
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11J)
        # ==========================
        try:
            branch_data = get_case_table("A11J")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11J branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11J."}
...
A11J_outputs.output_type = "branch_main"
