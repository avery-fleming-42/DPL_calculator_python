...
def A11D_outputs(stored_values, *_):
    """
    Calculate outputs for A11D case.
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11D)
        # ==========================
        try:
            branch_data = get_case_table("A11D")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11D branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11D."}
...
A11D_outputs.output_type = "branch_main"
