...
def A11R_outputs(inputs, data):
    ...
    try:
        ...
        # --- Branch Loss Coefficient (A11R) ---
        try:
            df_branch = data.loc["A11R"]
            branch_data = df_branch[df_branch["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11R branch data not found in Excel."}
