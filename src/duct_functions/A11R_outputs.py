...
def A11R_outputs(stored_values, *_):
    """
    Calculate outputs for A11R case.
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11R, PATH='branch')
        # ==========================
        try:
            branch_data = get_case_table("A11R")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11R branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11R (branch path)."}
        ...
        # ==========================
        #   MAIN LOSS COEFFICIENT (A11R, PATH='main')
        # ==========================
        try:
...
A11R_outputs.output_type = "branch_main"
