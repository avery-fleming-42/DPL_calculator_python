...
def _loss_coeff_nearest(angle_data: pd.DataFrame, angle: float, area_ratio: float) -> float:
    """
    Original method for SMACNA-style table lookup:
    ...
    # Match angle (smallest ANGLE >= angle, else max)
    ...
    # Match A1/A within that angle slice (smallest A1/A >= area_ratio, else max)
...
def _interp_c_at_angle(angle_data: pd.DataFrame, angle_slice: float, area_ratio: float) -> float:
    ...
    # Failsafe: if this exact angle is missing (shouldn't happen in normal flow),
    # fall back to the closest angle in the table.
    if slice_df.empty:
...
def _loss_coeff_bilinear(angle_data: pd.DataFrame, angle: float, area_ratio: float) -> float:
    ...
    - First, linearly interpolate C vs A1/A at the two bounding ANGLEs.
...
def A8H_outputs(stored_values, *_):
    """
    A8H: [update with exact SMACNA name]

    Uses bilinear interpolation for the loss coefficient instead of
    stepwise rounding to the nearest tabulated value.
    ...
    if not {"ANGLE", "A1/A", "C"}.issubset(df.columns):
        raise KeyError("Data for A8H must include 'ANGLE', 'A1/A', and 'C' columns.")
...
A8H_outputs.output_type = "standard"
