...
def A11H_outputs(stored_values, *_):
    """
    Calculate outputs for A11H case.
    ...
    try:
        ...
        Qc = stored_values.get("entry_3")
        Qb = stored_values.get("entry_4")
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11H)
        # ==========================
        try:
            branch_data = get_case_table("A11H")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11H branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11H."}
...
A11H_outputs.output_type = "branch_main"
