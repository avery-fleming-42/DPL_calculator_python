...
def A8I_outputs(stored_values, data):
    """
    Calculates the outputs for case A8I using the stored input values.
    ...
    if "A8I" not in data.index:
        raise KeyError("Data must include 'A8I' index for calculations.")
    df = data.loc["A8I"]
    ...
    if "ANGLE" not in df.columns or "A1/A" not in df.columns or "C" not in df.columns:
        raise KeyError("Data for A8I must include 'ANGLE', 'A1/A', and 'C' columns.")
