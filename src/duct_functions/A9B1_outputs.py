...
def A9B1_outputs(stored_values, data):
    """
    Calculates the outputs for case A9B1 using the stored input values.
    ...
    if "A9B1" not in data.index:
        raise KeyError("Data must include 'A9B1' index for calculations.")
    df = data.loc["A9B1"]
    ...
    velocity = entry_5 / area_1  # Velocity in ft/min

    # Calculate L/D
    length_diameter_ratio = entry_3 / entry_1
    ...
    if "L/D" not in df.columns or "ANGLE" not in df.columns or "C" not in df.columns or "A/A1" not in df.columns:
        raise KeyError("Data for A9B1 must include 'L/D', 'ANGLE', 'C', and 'A/A1' columns.")
    ...
    valid_angle = angle_data[angle_data["ANGLE"] <= entry_4] if entry_4 < 60 else angle_data[angle_data["ANGLE"] >= entry_4]
    if valid_angle.empty:
        closest_angle_row = angle_data.iloc[0] if entry_4 < 60 else angle_data.iloc[-1]  # Round down or up based on angle
    else:
        closest_angle_row = valid_angle.iloc[0] if entry_4 < 60 else valid_angle.iloc[-1]
