...
def A11E_outputs(stored_values, *_):
    """
    Calculate outputs for A11E case.
    ...
    try:
        ...
        Qc = stored_values.get("entry_3")  # Converged / upstream flow
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11E)
        # ==========================
        try:
            branch_data = get_case_table("A11E")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11E branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11E."}
...
A11E_outputs.output_type = "branch_main"
