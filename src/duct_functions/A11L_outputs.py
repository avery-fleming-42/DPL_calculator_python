...
def A11L_outputs(stored_values, *_):
    """
    Calculate outputs for A11L case.
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11L)
        # ==========================
        try:
            branch_data = get_case_table("A11L")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11L branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11L."}
...
A11L_outputs.output_type = "branch_main"
