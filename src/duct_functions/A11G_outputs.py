...
def A11G_outputs(stored_values, *_):
    """
    Calculate outputs for A11G case.
    ...
    try:
        ...
        # ==========================
        #   BRANCH LOSS COEFFICIENT (A11G)
        # ==========================
        try:
            branch_data = get_case_table("A11G")
            branch_data = branch_data[branch_data["PATH"] == "branch"].copy()
        except KeyError:
            return {"Error": "A11G branch data not found."}

        if branch_data.empty:
            return {"Error": "No branch data found for A11G."}
...
A11G_outputs.output_type = "branch_main"
