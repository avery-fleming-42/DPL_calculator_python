...
def A7H2_outputs(stored_values, *_):
    """
    A7H2 – Mitered with Double Thickness Turning Vanes (Rectangular).
    ...
    Rules:
        ...
        - Loss coefficient C is taken from the A7H2 table:
    ...
    # If both given, prefer R and ignore S
    if R_in is not None and S_in is not None:
        # print("[INFO] A7H2: both R and S entered; using R and ignoring S.")
        S_in = None
    ...
    try:
        ...
        #  Load A7H2 table
        # -----------------------------
        df = get_case_table("A7H2").copy()

        needed_cols = ["R (in)", "S (in)", "V, fpm", "C"]
        for col in needed_cols:
            if col not in df.columns:
                raise KeyError(f"Required column '{col}' not found in A7H2 table.")
        ...
        if dim_subset is None or dim_subset.empty:
            return {
                ...
                "Output 3: Loss Coefficient": None,
                "Output 4: Pressure Loss (in w.c.)": None,
                "Error": "No matching R/S geometry found in A7H2 table.",
            }
        ...
        if not vel_values:
            return {
                ...
                "Output 3: Loss Coefficient": None,
                "Output 4: Pressure Loss (in w.c.)": None,
                "Error": "No velocity values found in A7H2 table.",
            }
...
A7H2_outputs.output_type = "standard"