...
def A8F_outputs(stored_values, data):
    """
    Calculates the outputs for case A8F using the stored input values.
    ...
    if "A8F" not in data.index:
        raise KeyError("Data must include 'A8F' index for calculations.")
    df = data.loc["A8F"]
    ...
    if "ANGLE" not in df.columns or "A1/A" not in df.columns or "C" not in df.columns:
        raise KeyError("Data for A8F must include 'ANGLE', 'A1/A', and 'C' columns.")
