...
def A9A1_outputs(stored_values, data):
    """
    Calculates the outputs for case A9A1 using the stored input values.
    ...
    if "A9A1" not in data.index:
        raise KeyError("Data must include 'A9A1' index for calculations.")
    df = data.loc["A9A1"]

    # Calculate velocity using downstream diameter (D₁)
    area_1 = math.pi * (entry_2 / 2) ** 2 / 144  # Area 1 in square feet
    velocity = entry_4 / area_1  # Velocity in ft/min
    ...
    if "ANGLE" not in df.columns or "A1/A" not in df.columns or "C" not in df.columns:
        raise KeyError("Data for A9A1 must include 'ANGLE', 'A1/A', and 'C' columns.")
