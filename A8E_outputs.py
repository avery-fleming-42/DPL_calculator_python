...
def A8E_outputs(stored_values, data):
    """
    Calculates the outputs for case A8E using the stored input values.
    ...
    if "A8E" not in data.index:
        raise KeyError("Data must include 'A8E' index for calculations.")
    df = data.loc["A8E"]
    ...
    if "ANGLE" not in df.columns or "A1/A" not in df.columns or "C" not in df.columns:
        raise KeyError("Data for A8E must include 'ANGLE', 'A1/A', and 'C' columns.")
    ...
    if entry_4 < 90:
        ...
        if valid_angle.empty:
            ...
            closest_angle_row = valid_angle.iloc[-1]  # Use the largest ANGLE less than or equal to entry_4
